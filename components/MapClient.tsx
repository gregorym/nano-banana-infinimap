...
export default function MapClient() {
  ...
  const pendingChecksRef = useRef<Record<string, Promise<boolean>>>({});
  ...
  // Check if a tile exists with memoization to prevent duplicate requests
  const checkTileExists = useMemo(() => {
    return async (x: number, y: number) => {
      try {
        const key = `${x},${y}`;

        // Return cached result if available
        if (tileExists[key] !== undefined) {
          return tileExists[key];
        }

        // Return existing promise if request is already in flight
        if (await pendingChecksRef.current[key]) {
          return pendingChecksRef.current[key];
        }
        ...
        const promise = (async () => {
          try {
            ...
            setTileExists((prev) => ({ ...prev, [key]: exists }));
            return exists;
          } finally {
            // Clean up pending request
            delete pendingChecksRef.current[key];
          }
        })();

        // Store the promise to prevent duplicate requests
        pendingChecksRef.current[key] = promise;
        return promise;
      } catch {
        delete pendingChecksRef.current[`${x},${y}`];
        return false;
      }
    };
  }, [tileExists]);
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      // Track mouse hover over tiles with debouncing
      m.on("mousemove", (e: any) => {
        if (mouseMoveTimeoutRef.current) {
          clearTimeout(mouseMoveTimeoutRef.current);
        }

        mouseMoveTimeoutRef.current = setTimeout(async () => {
      ...
      <div className="p-3 z-10 absolute top-2 left-2 bg-white/90 rounded-xl shadow-lg flex flex-col gap-2">
        ...
        {searchParams.get("z") && (
          <div className="text-xs text-gray-400">
            Position: z={searchParams.get("z")}, lat={searchParams.get("lat")},
        ...
        >
          ...
          >
            ...
            <TileControls
              ...
              onGenerate={(prompt) =>
                handleGenerate(selectedTile.x, selectedTile.y, prompt)
              }
              onRegenerate={(prompt) =>
                handleRegenerate(selectedTile.x, selectedTile.y, prompt)
              }
